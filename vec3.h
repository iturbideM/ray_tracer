...
inline vec3 random_unit_vector() {
    while (true) {
        ...
        // Reject points that are too close to zero to avoid underflow to zero when squared
...
inline vec3 random_on_hemisphere(const vec3& normal) {
    ...
    if (dot(on_unit_sphere, normal) > 0.0) // In the sasme hemisphere as the normal
