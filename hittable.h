...
class rotate_y : public hittable {
  public:
    ...
    bool hit(const ray& r, interval ray_t, hit_record& rec) const override {
        // Transform the ray form world space to object space
        auto origin =
        ...
        // Determine wheter an intersection exists in object space (and if so, where)/
...
class rotate_x : public hittable {
  public:
    ...
    bool hit(const ray& r, interval ray_t, hit_record& rec) const override {
        // Transform the ray form world space to object space
        auto origin =
            point3(r.origin().x(), (cos_theta * r.origin().y()) - (sin_theta * r.origin().z()),
        ...
        rec.normal =
            vec3(rec.normal.x(), (cos_theta * rec.normal.y()) - (sin_theta * rec.normal.z()),
